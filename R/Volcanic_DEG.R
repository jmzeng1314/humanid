...
Volcanic_DEG <- function(DEG,imageType='png',prefix='test'){
  ...
  library(calibrate)
  with(subset(res, adj.P.Val<.05 & abs(logFC)>1), textxy(logFC, -log10(P.Value), labs=Gene, cex=.8))
  dev.off()


  logFC_Cutof <- with(res,mean(abs( logFC)) + 2*sd(abs( logFC)) )
  res$change = as.factor(ifelse(res$P.Value < 0.05 & abs(res$logFC) > logFC_Cutof,ifelse(res$logFC > logFC_Cutof ,'UP','DOWN'),'NOT'))
  ...
  g = ggplot(data=res, aes(x=logFC, y=-log10(P.Value), color=change)) +
