...
#'
#'
pub_trend <- function(years = 2010:2016,
                      keywords = c("3C","4C","5C","Hi-C","ChIA???PET","3D chromosome")
){
  library(rentrez)
  library(ggplot2)
  library(reshape2)
  # papers_by_year function
  papers_by_year<- function(years,search_term){return(sapply(years,function(y)entrez_search(db="pubmed",term=search_term,mindate=y,maxdate=y,retmax=0)$count))}
  total_papers<- papers_by_year(years,"");total_papers
