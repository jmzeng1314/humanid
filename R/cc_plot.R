...
cc_plot  <- function(expr, meta, ps=8, fs=23){
  ...
  plt=ggplot(dfp)+labs(title='Correlation Plot: Within Group (red line) and Overall (blue line)')+
    geom_point(aes(x=Sample, y=cc_grp, color=Group), size=ps)+
    geom_line(aes(x=sid, y=cc_grp), color='red', size=1)+
    geom_point(aes(x=Sample, y=cc_all, color=Group), shape=17, size=ps)+
    geom_line(aes(x=sid, y=cc_all), color='blue', size=1)+
    scale_color_manual(values=unique(meta$Color))+ylab('Corr. Coefficient')+
    theme(axis.text.x=element_text(angle=45, hjust=1), text=element_text(size=fs))
  #annotate('text', hjust=0, x=length(smp)/2, y=0.98, label='- Within Group', color='red')
  #annotate('text', hjust=0, x=length(smp)/2, y=0.95, label='- Overall', color='blue')
