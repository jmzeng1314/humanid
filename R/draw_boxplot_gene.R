...
draw_boxplot_gene <- function(gene = "VCX3A", exprSet, group_list) {
    ...
    if (gene %in% rownames(exprSet)) {
        ...
        dat1 = data.frame(value = as.numeric(exprSet[gene, ]), type = group_list)
        boxplot(value ~ type, data = dat1, lwd = 2, ylab = "value")
        title(main = gene, sub = t.test(value ~ type, data = dat1)$p.value)
        stripchart(value ~ type, vertical = TRUE, data = dat1, method = "jitter", add = TRUE, pch = 20, col = "blue")
        dev.off()
    }
}
# CTlist=read.table('CTlist.txt',stringsAsFactors = F)[,1] lapply(CTlist, function(x)
# draw_boxplot_gene(x,exprSet,group_list) )
